...
onready var exp_bar = $Inventory_Frame/EXP_Bar
onready var exp_amount = $Inventory_Frame/EXP_Bar/EXP_Amount
...
func update_exp():
	...
	var tween = get_tree().create_tween()
	tween.tween_property(exp_bar,"value",float(which_goat_node.goat_exp),.2)
	exp_amount.text = "%s/%s - lvl %s" %[
		which_goat_node.goat_exp,
		which_goat_node.goat_next_exp,
		which_goat_node.goat_level
		]
	
func update_energy(save_choice):	
	if save_choice: which_goat_node.save_goat()
...
func _on_train_button_pressed():
	...
	if which_goat_node.goat_current_energy >= training_cost:
		...
		Global.MAIN.remove_scene("training_menu",0)
	else:
