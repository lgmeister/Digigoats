...
onready var portrait = $Balloon/Character_Portrait
...
func _ready() -> void:
	if Global.current_npc.NPC_portrait != null:
		portrait.texture = Global.current_npc.NPC_portrait
...
func _on_response_gui_input(event, item):
	...
	if event is InputEventMouseButton and event.is_pressed() and event.button_index == 1:
		...
		AUDIO.play("select")
	elif event.is_action_pressed("action") and item in get_responses():
		next(dialogue_line.responses[item.get_index()].next_id)
		AUDIO.play("select")
		
	if event is InputEventKey and event.is_action_pressed("movement_keys"):
		AUDIO.play("blip")
...
func _on_Balloon_gui_input(event):
	...
	if event is InputEventMouseButton and event.is_pressed() and event.button_index == 1:
		next(dialogue_line.next_id)
	elif event.is_action_pressed("action") and balloon.get_focus_owner() == balloon:
		next(dialogue_line.next_id)
