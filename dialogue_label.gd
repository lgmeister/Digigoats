...
var letter_noise_skip: int = 3
var letter_noise_iteration: int = 0
	...
	if is_typing:
		...
		if percent_visible < 1:
			...
			if waiting_seconds <= 0:
				...
				###### TALK SOUNDS ######
				letter_noise_iteration += 1
				if letter_noise_iteration == letter_noise_skip:
					AUDIO.play_dialog(Global.current_npc.NPC_talk_voice,0)
				if letter_noise_iteration >= letter_noise_skip:
					letter_noise_iteration = 0
				###### TALK SOUNDS ######
